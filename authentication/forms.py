...
class SignupForm(forms.ModelForm):
    ...
    confirmPassword = forms.CharField(widget=forms.PasswordInput(attrs={
        ...
        RegexValidator('^.*(?=.{8,})(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[@#$%^&+=]).*$', message="password too week")
    ])
    ...
    def clean(self):
        ...
        val_confirm_password = self.cleaned_data.get("confirmPassword")
    ...
    class Meta:
        model = Register
        fields = ('first_name', 'last_name',  'email','password', 'confirmPassword', 'phone', 'image')
